...
func (r *Repository) FindAllMyFarm(status, uid string) ([]model_databases.Farm, error) {
	...
	subQuery := r.db.Debug().Select("farm_id").Where("status_id = ? AND uid = ?",config.GetStatus().Active, uid).Table(config.DB_TRANS_MANAGEMENT)
	resp := r.db.Debug().Where("status_id = ? AND farm_id IN (?)", status, subQuery).Find(&result)
